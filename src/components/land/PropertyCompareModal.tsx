            ...
            <table className="w-full min-w-[700px] border-collapse text-left text-xs">
              <thead>
                <tr className="border-b border-brand-border">
                  <th className="p-3 font-bold text-brand-muted w-36">Feature</th>
                  {comparedProperties.map((prop) => (
                    <th key={prop.id} className="p-3 font-bold text-brand-ink w-56">
                      <div className="relative aspect-[4/3] rounded-xl overflow-hidden mb-2 border border-brand-border">
                        ...
                        <button
                          ...
                          className="absolute top-1.5 right-1.5 p-1 rounded-full bg-black/60 text-white hover:bg-brand-clay transition-colors"
                          title="Remove"
                        >
              ...
              <tbody className="divide-y divide-brand-border/60">
                ...
                <tr className="bg-brand-forest-light/30">
                  <td className="p-3 font-bold text-brand-forest">Monthly Payment</td>
                  {comparedProperties.map((prop) => (
                    <td key={prop.id} className="p-3 font-extrabold text-brand-forest text-sm">
                ...
                {/* Down Payment */}
                <tr>
                  <td className="p-3 font-semibold text-slate-600">Down Payment</td>
                  {comparedProperties.map((prop) => (
                    <td key={prop.id} className="p-3 font-bold text-brand-ink">
                ...
                <tr>
                  <td className="p-3 font-semibold text-slate-600">Discounted Cash Price</td>
                  {comparedProperties.map((prop) => (
                    <td key={prop.id} className="p-3 font-bold text-brand-ink text-sm">
                ...
                {/* Acreage */}
                <tr>
                  <td className="p-3 font-semibold text-slate-600">Acreage</td>
                  {comparedProperties.map((prop) => (
                    <td key={prop.id} className="p-3 font-bold text-brand-ink">
                ...
                {/* Road Access */}
                <tr>
                  <td className="p-3 font-semibold text-slate-600">Road Access</td>
                  {comparedProperties.map((prop) => (
                    <td key={prop.id} className="p-3 text-slate-700">
                ...
                {/* Utilities */}
                <tr>
                  <td className="p-3 font-semibold text-slate-600">Utilities</td>
                  {comparedProperties.map((prop) => (
                    <td key={prop.id} className="p-3 space-y-1">
                ...
                {/* 360 View */}
                <tr>
                  <td className="p-3 font-semibold text-slate-600">360° Virtual Tour</td>
                  {comparedProperties.map((prop) => (
                  ...
                  {comparedProperties.map((prop) => (
                    <td key={prop.id} className="p-3">
                      <Link
                        href={`/products/${prop.handle}`}
                        onClick={() => setIsCompareModalOpen(false)}
                        className="inline-flex items-center justify-center gap-1.5 w-full bg-brand-ink text-white font-semibold py-2 px-3 rounded-xl hover:bg-brand-forest transition-colors text-xs"
                      >
